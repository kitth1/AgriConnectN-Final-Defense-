...
<?php
    ...
    // Ensure the user is logged in and the session variable for barangay is set
    if (!isset($_SESSION['tech_username']) || !isset($_SESSION['tdesignation'])) {
        echo "<p>User barangay information not available. Please log in.</p>";
        // Optionally, redirect to a login page or another page
        // header('Location: login.php');
        // exit();
    } else {
        // Retrieve the barangay from session
        $barangay = $_SESSION['tdesignation']; 

        // Display the barangay name
        echo "<h2>BARANGAY " . htmlspecialchars($barangay) . "</h2>";
    }
    ?>
...
<table class="table">
    ...
    <tbody>
        ...
        $sql = "SELECT * FROM farmer_acc2 WHERE barangay= 'Agusipan' ORDER BY barangay ASC";
